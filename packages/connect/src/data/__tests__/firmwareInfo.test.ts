import { getReleases, parseFirmware, getFirmwareStatus, DeviceModel } from '../firmwareInfo';
import * as releases2 from '@trezor/connect-common/files/firmware/t2t1/releases.json';

describe('data/firmwareInfo', () => {
    beforeEach(() => {
        parseFirmware(releases2, DeviceModel.T2T1);
    });

    test('getReleases', () => {
        expect(getReleases(DeviceModel.T2T1)[0]).toMatchObject({
            ...releases2[0],
